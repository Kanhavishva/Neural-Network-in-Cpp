...
#define MATPLOTLIBCPP_PYTHON_HEADER /usr/include/python3.5m/Python.h  // <<------------------ I modified this line
...
namespace matplotlibcpp {
    ...
    void plot_matrix(gsl_matrix * mat)
    {
        std::vector<double> array (mat->size2);

        for(int i = 0; i < (int)mat->size1; i++)
        {
            for(int j = 0; j < (int)mat->size2; j++)
            {
                array.at (j) = gsl_matrix_get(mat, i, j);
            }
            plot(array);
        }
        show ();
